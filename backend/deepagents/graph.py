...
def get_default_model() -> BaseChatModel:
    """Get a default model for deep agents.

    Pluto Duck note:
    - This repository vendors deepagents but does NOT vendor provider-specific integrations
      (e.g. Anthropic prompt caching).
    - In production we pass an explicit model instance (or wrapper) from Pluto Duck.
    """
    raise ValueError(
        "No default model configured in vendored deepagents for Pluto Duck. "
        "Pass an explicit `model` (BaseChatModel) or a model string supported by `init_chat_model()`."
    )
